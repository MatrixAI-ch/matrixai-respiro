...
var smartAssistantId = 6543c6c3cc4db1db0d21fcb4; //document.currentScript.id;
if (!smartAssistantId) {
    alert("Chatbot id is not defined.");
    throw new Error("Chatbot id is not defined.")
}
main();
async function main() {
    ...
    if (smartAssistantBubbleData.status === FORBIDDEN_CODE) {
        alert("This chatbot's accessibility is protected. You must change it to public or private in your chatbot's settings.");
        return
    } else if (smartAssistantBubbleData.status === NOT_FOUND_CODE) {
        alert("This chatbot does not exist.");
        return
    }
    ...
    if (!isAllowed) {
        alert("This domain is not allowed. Add this domain to the allowed domain list in your chatbot settings.");
        return
    }
    loadFontAwesome();
